...
export const insertTransactionSchema = z.object({
  ...
  category: z.enum(['Material', 'Mão de obra', 'Aluguel', 'Outros']).nullable().optional(),
...
export async function updateTransaction(id: number, data: Partial<InsertTransaction>) {
  ...
  const result = await db.update(transactions).set({ ...validated, updatedAt: new Date().toISOString() }).where(eq(transactions.id, id)).returning();
  return result[0];
}
