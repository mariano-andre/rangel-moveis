...
const UNITS: InventoryUnit[] = ["chapas", "unid.", "kg", "caixa", "metro", "litro"];
...
function EntryForm({ items, onClose, onSave }: Omit<EntryModalProps, "mode">) {
  ...
  function validate() {
    ...
    if (!quantity || parseFloat(quantity) <= 0)   e.quantity = "Informe a quantidade.";
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {/* Material */}
      <div>
        <label className="block text-xs text-text-muted uppercase tracking-wide mb-1.5">Material</label>
        <select
          value={selectedId}
          onChange={(e) => {
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div>
          <label className="block text-xs text-text-muted uppercase tracking-wide mb-1.5">Preço/un. (R$)</label>
          <input
            type="number"
            min="0"
            step="0.01"
      ...
      <div className="flex justify-end gap-2 pt-1">
        ...
        <Button variant="primary" onClick={handleSave}>Registrar entrada</Button>
      </div>
...
function EditForm({ item, onClose, onSave }: Omit<EditModalProps, "mode">) {
  ...
  function validate() {
    ...
    if (!form.quantity || parseFloat(form.quantity) < 0) e.quantity = "Informe a quantidade.";
    if (!form.minimum  || parseFloat(form.minimum)  < 0) e.minimum  = "Informe o estoque mínimo.";
    setErrors(e);
    return Object.keys(e).length === 0;
  }
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          />
          {errors.material && <p className="text-xs text-danger mt-1">{errors.material}</p>}
        </div>
        <div>
          <label className="block text-xs text-text-muted uppercase tracking-wide mb-1.5">Unidade</label>
          <select value={form.unit} onChange={(e) => set("unit", e.target.value)} className={inputClass(false)}>
      ...
      <div className="grid grid-cols-3 gap-3">
        <div>
          <label className="block text-xs text-text-muted uppercase tracking-wide mb-1.5">Qtd. atual</label>
          <input
        ...
        <div>
          <label className="block text-xs text-text-muted uppercase tracking-wide mb-1.5">Mínimo</label>
          <input
            type="number" min="0" step="0.01"
            value={form.minimum}
          ...
          />
          {errors.minimum && <p className="text-xs text-danger mt-1">{errors.minimum}</p>}
        </div>
        <div>
          <label className="block text-xs text-text-muted uppercase tracking-wide mb-1.5">Preço/un.</label>
          <input
            type="number" min="0" step="0.01"
...
export function InventoryModal(props: InventoryModalProps) {
  const title = props.mode === "entry" ? "Registrar entrada" : "Editar material";

  return (
    <Modal title={title} onClose={props.onClose}>
      {props.mode === "entry"
        ? <EntryForm items={props.items} onClose={props.onClose} onSave={props.onSave} />
        : <EditForm  item={props.item}   onClose={props.onClose} onSave={props.onSave} />
      }
    </Modal>
  );
}
