...
export const settings = sqliteTable('settings', {
  ...
  alertLowInventory: integer('alert_low_inventory', { mode: 'boolean' }).notNull(),
  alertDeadlineApproaching: integer('alert_deadline_approaching', { mode: 'boolean' }).notNull(),
  alertPendingPayment: integer('alert_pending_payment', { mode: 'boolean' }).notNull(),
  alertWeeklyFinancialSummary: integer('alert_weekly_financial_summary', { mode: 'boolean' }).notNull(),
  createdAt: text('created_at').notNull().$defaultFn(() => new Date().toISOString()),
  updatedAt: text('updated_at').notNull().$defaultFn(() => new Date().toISOString()),
});
